...
* `-specs=hupair.specs`\
プログラム実行開始時のコマンドライン引数受け渡し処理をHUPAIR規格に対応させます。
HUPAIR規格に対応したシェルからプログラムを実行した場合に、255バイトを超えるコマンドライン引数を受け取ることができるようになります。
HUPAIR規格については[HUPAIR規格書](http://retropc.net/x68000/software/system/hupair/hupair_document/)を参照してください。
...
m68k用gccはコンパイル、リンク時のオプション指定 `-m68000` , `-m68010` , `-m68020` , `-m68030` , `-m68040` , `-m68060` によって、CPUの種類を指定することができます。
また、`-msoft-float`, `-mhard-float` オプションによって浮動小数点演算にFPU命令を使用するかどうかを指定することができます。これらのオプションが指定された場合は、Newlib も対応したオプションでコンパイルされたライブラリがリンクされます。

これらのオプションは、X68030 や 040turbo/060turbo などの 68000 以外のCPUを搭載した環境専用に最適化されたバイナリを生成するのに有用ですが、一方でこうしたバイナリを未対応の環境で実行させると、サポートされていない命令を実行した時点で不当命令例外が発生してプログラムが異常終了することになります。

elf2x68k の Newlib 環境では、プログラムの実行開始時に実行環境がコンパイル時のCPU, FPU指定に対応しているかどうかを判別する機能を追加しています。もし対応していないバイナリを実行すると、`MPU/FPU mismatch. Program terminated.` というエラーメッセージを表示して実行を終了します。
