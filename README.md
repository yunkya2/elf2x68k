...
* 最新の binutils (2.42)、gcc (14.1.0) を用いた X68k のクロス開発が可能です
  * gcc-14 対応については下記「GCC 14への対応について」も参照してください
...
**elf2x68k は最新版 gcc-14 に対応しました。**
gcc-14 では、以下のような従来 warning として扱われてきた記述がエラー扱いになります。このため、ソースコードに適切な修正が必要になる場合があります。\
また、gcc のコマンドラインオプション `-fpermissive` の指定によって、gcc-13 以前と同様の挙動に戻すことができます。

* 暗黙的な int 型宣言
  * 例: `func(a, b) { ... }` は `int func(int a, int b) { ... }` とする必要があります
* 暗黙的な関数宣言
  * プロトタイプ宣言のない関数呼び出しがエラーとなります
  * 例: `strlen()` を呼び出す場合、事前に `#include <string.h>` が必要です
* return 文の不正な使用
  * void 型関数から値を持つ `return` 文で戻ったり、void 型以外の型の関数から値のない `return` 文で戻ることはできません
* 型の異なるポインタ同士や整数型との間の代入
  * 適切なキャストを行う必要があります

詳細については、[Porting to GCC14](https://gcc.gnu.org/gcc-14/porting_to.html) を参照してください(英語)。
